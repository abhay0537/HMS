...
const signin = async (req, res) => {
  try {
    ...
    if (!username || !password) {
      ...
      if (!auth_user) {
        return res.status(401).json({ message: `user with ${username} no found` });
      } else {
...
const doctorsignin = async (req, res) => {
  try {
    ...
    if (!email || !password) {
      ...
      if (!auth_user) {
        return res.status(401).json({ message: `user with ${email} no found` });
      } else {
        const verify = await bcryptjs.compare(password, auth_user.password);
        if (!verify) {
  ...
  try {
    ...
    if (!username || !password || !email) {
      ...
      if (!new_user) {
        ...
        await user.create({ username, password: hashed_password, email, gender, age, location, phone });
...
module.exports = { signin, signup, doctorsignin };
