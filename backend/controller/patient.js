...
const single_user = async (req, res) => {
  ...
  try {
    ...
    return res.status(202).json({ message: "find user successfully", data });
  } catch (e) {
    return res.status(400).json({ message: e.message });
  }
};
...
const update_user = async (req, res) => {
  const id = req.id;
  let { username, email, password, phone, gender, age, location, image } = req.body;
  let updateobject = {};

  if (password) {
    password = await bcryptjs.hash(password, 10);
    updateobject = { username, email, password, phone, gender, age, location, image };
  } else {
    updateobject = { username, email, phone, gender, age, location, image };
  }
  ...
  try {
    ...
    return res.status(202).json({ message: "update successfully", data: updateduser });
  } catch (e) {
    return res.status(400).json({ message: e.message });
  }
};
